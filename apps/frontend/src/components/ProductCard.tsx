...
const ProductCard: React.FC<ProductCardProps> = ({ image, title, price, platform, onInvestClick }) => (
  <div className="bg-white rounded-xl shadow-lg p-5 flex flex-col items-center transition-transform hover:scale-105 hover:shadow-2xl border border-gray-100">
    ...
    <h3 className="font-semibold text-lg text-gray-800 mb-1 text-center line-clamp-2">{title}</h3>
    <p className="text-base font-bold text-indigo-600 mb-1">₹{price}</p>
    <span className="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded mb-2">{platform}</span>
    <button
