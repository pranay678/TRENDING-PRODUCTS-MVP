...
const Modal: React.FC<{ open: boolean; onClose: () => void; product: any }> = ({
  open,
  onClose,
  product,
}) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-green-500 to-indigo-500 text-white font-semibold py-2 rounded-lg shadow hover:from-green-600 hover:to-indigo-600 transition-colors"
            disabled={loading}
          >
...
const HomePage: React.FC = () => {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const inCategory =
      category === 'All' ||
      (product.category ? product.category === category : true);
